...
int main()
{
    ...
    while(cin >> n >> q && n && q)
    {
        ...
        rep(i, 0, n)
        {
            total = total + (vec[i].x + vec[j].x) * (vec[j].y - vec[i].y);
            hoba.push_back(total);
            j = i;
        }
        ...
        while(q--)
        {
            ...
            temp = temp + (vec[v].x + vec[u].x) * (vec[u].y - vec[v].y);

            j = v;

            double extra = hoba[n-1] - hoba[v];

            temp += extra;

            temp /= 2;

            cout << fixed << setprecision(1) << min(temp, total - temp) << endl;
        }
    }
