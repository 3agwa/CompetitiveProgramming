...
int main()
{
    ...
    while(cin >> n && n >= 0)
    {
        ...
        rep(i, 0, n)
        {
            ...
            {
                if (i == j) continue;
                if (vec[i] == vec[j] && r[i] == r[j]) node[i].push_back(j);
                ld temp = dist2(vec[i], vec[j]);
                if (r[i]+r[j] >= temp && temp >= fabs(r[i]-r[j])) node[i].push_back(j);
            }
        }
        int mx = 0;
        rep(i, 0, n)
        {
            if (!visited[i])
            {
                ...
                while(!Q.empty())
                {
                    ...
                    visited[hoba] = true;
                    cnt++;
                    rep(j, 0, sz(node[hoba]))
                    {
                        if (!visited[node[hoba][j]]) Q.push(node[hoba][j]);
                    }
                }
                mx = max(mx, cnt);
            }
        }
        if (mx == 1) cout << "The largest component contains 1 ring.\n";
        else cout << "The largest component contains " << mx << " rings.\n";
    }
