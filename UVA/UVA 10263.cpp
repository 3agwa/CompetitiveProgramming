...
/*
    finding projection of point M on each segment and minimizing the distance depending on the projection
*/
...
int main()
{
    ...
    while(cin >> m)
    {
        ...
        cin >> n;
        vector<PT> vec(n+1);
        erep(i, 0, n) cin >> vec[i];
        PT temp;
        double mx = 1e10, x = -1, y = -1;
        rep(i, 0, n)
        {
            temp = ProjectPointSegment(vec[i], vec[i+1], m);
            if (dist2(m, temp) < mx)
            {
                mx = dist2(m, temp);
                x = temp.x;
